...
async def set_group_photo(gpic):
    """ For .gp command, changes the picture of a group """
...
async def promote(promt):
    """ For .pro command, promotes the replied/tagged person """
    # Get targeted chat
...
async def demote(dmod):
    """ For .dem command, demotes the replied/tagged person """
    # Admin right check
...
async def ban(bon):
    """ For .b command, bans the replied/tagged person """
...
async def nothanos(unbon):
    """ For .u command, unbans the replied/tagged person """
    # Here laying the sanity check
...
async def unmoot(unmot):
    """ For .um command, unmute the replied/tagged person """
    # Admin or creator check
...
async def ungmoot(un_gmute):
    """ For .ugm command, ungmutes the target in the userbot """
    # Admin or creator check
...
async def gspider(gspdr):
    """ For .gm command, globally mutes the replied/tagged person """
    # Admin or creator check
...
async def rm_deletedacc(show):
    """ For .zo command, list all the ghost/deleted/zombie accounts in a chat. """
...
async def get_admin(show):
    """ For .ads command, list all of the admins of the chat. """
...
async def kick(usr):
    """ For .k command, kicks the replied/tagged person from the group. """
    # Admin or creator check
...
async def get_users(show):
    """ For .us command, list all of the users in a chat. """
    info = await show.client.get_entity(show.chat_id)
    title = info.title if info.title else "this chat"
    mentions = 'Users in {}: \n'.format(title)
    try:
...
async def get_usersdel(show):
    """ For .ud command, list all of the deleted users in a chat. """
    info = await show.client.get_entity(show.chat_id)
    title = info.title if info.title else "this chat"
...
\n.m & .um Mute & Unmute\
\n.pin Pinned Messages\
\n.lo & .unl Lock & Unlock all, msg, media, sticker,\
\ngif, game, inline, poll, invite, pin, info."
})
